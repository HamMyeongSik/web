...
// 현재 자산
let currentMoney = 50000000; // 예시로 1500만원으로 설정
...
function createPlayerDiv(player, index) {
    ...
    playerDiv.style.boxShadow = "0 4px 8px rgba(0,0,0,0.1)";
    ...
    // 추가된 부분: 선수 목록 상단에 팀 목록과 현재 자산 표시
    const teamListAndAsset = document.createElement("div");
    teamListAndAsset.style.display = "flex";
    teamListAndAsset.style.justifyContent = "space-between";
    teamListAndAsset.style.marginBottom = "10px";
...
function generateRandomPlayer(price) {
    ...
    const randomImage = playerImages[randomName];
